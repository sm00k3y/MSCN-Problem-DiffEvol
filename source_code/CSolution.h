...
public:
	CSolution();
	CSolution(unsigned int iSuppliersCount, unsigned int iFactoriesCount, unsigned int iWarehousesCount, unsigned int iShopsCount, bool& bSuccess);
	~CSolution();
	CSolution(const CSolution& cOther);
	CSolution& operator=(const CSolution& cOther);
	...
	void vInitTableFromMatrixes();
	void vInitMatrixesFromTable();

	bool bIdentical(CSolution& cOther);

private:
	bool bSetSolutionMatrix(CMatrix& matrixToSet, int iIndex, int jIndex, int& iSolutionIndex);
	void vInitTableTwoDim(CMatrix& matrixToInit, int iIndex, int jIndex, int& iSolutionIndex);
	void vInitMatrixTwoDim(CMatrix& matrixToInit, int iIndex, int jIndex, int& iSolutionIndex);

	void vCopy(const CSolution& cOther);
